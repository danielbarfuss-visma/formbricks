...
## Discovery Context — Scheduled Follow-Ups (2026-05-15)

<!-- Non-inferable rationale only. Sourced from discovery-engine handover (Stage 7). -->
<!-- See: outputs/handover-scheduled-follow-ups-brief.md and outputs/survey-follow-ups-reference.md -->

### Why this feature exists
Survey creators needed delayed follow-up emails (e.g. "check in 7 days after submission").
The module had no scheduling layer — all sends fired synchronously in the pipeline request.

### Critical: fix the trigger evaluation condition before adding the new type
`apps/web/modules/survey/follow-ups/lib/follow-ups.ts:217` currently reads `if (trigger.properties)` — not `if (trigger.type === "endings")`.
Adding `"scheduled"` with non-null `properties` will silently route all scheduled follow-ups
through the endings filter, causing every one to skip. Change the condition to
`if (trigger.type === "endings")` as the first step.

### Rate limit must be applied in the cron processor — it is NOT inherited
`sendFollowUpsForResponse()` applies `applyRateLimit(rateLimitConfigs.actions.surveyFollowUp, orgId)`.
The cron processor calls `sendFollowUpEmail()` directly and bypasses this. Apply the same
rate limit check in the processor before each send.

### Entitlement check applies to scheduled sends too
Call `getSurveyFollowUpsPermission(organizationId)` in the cron processor.
Scheduled follow-ups share the existing `"follow-ups"` Stripe entitlement — no new tier.

### Cascade delete is required on ScheduledFollowUp.followUpId
`SurveyFollowUp` already cascades from `Survey`. Add `onDelete: Cascade` on
`ScheduledFollowUp.followUpId → SurveyFollowUp.id` or PENDING records will send
after the follow-up is deleted.

### delayDays = 7 is the product-decided default; range is 1–365
These are UX/product constraints, not technical ones. Do not derive them from the DB.

### No retry in v1 — FAILED status is terminal by decision
Failed scheduled sends are logged and left as FAILED. No queue, no auto-retry.
This is an explicit product decision, not an oversight.

### The pipeline DB write must stay fast
`sendFollowUpsForResponse` runs inside the respondent-facing HTTP request.
Creating a `ScheduledFollowUp` row is fine. Do not add rendering or external calls here.

### `toValueFromResponse[2]` (ContactInfo email index) is pre-existing and unchanged
Index 2 in the array is the email sub-field of a ContactInfo question response.
There is no named constant for this. The scheduled processor inherits this via `sendFollowUpEmail()`.
